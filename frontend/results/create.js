document.addEventListener('DOMContentLoaded', () => {
    populateStudents();
    populateCourses();
    document.getElementById('addResultForm').addEventListener('submit', handleAddResult);
});
...
async function handleAddResult(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            messageDiv.textContent = result.error || 'Failed to add result.';
        }
    } catch (error) {
        console.error('Error adding result:', error);
        const messageDiv = document.getElementById('message');
        messageDiv.className = 'message error';
        messageDiv.textContent = `Error adding result: ${error.message}`;
    }
}